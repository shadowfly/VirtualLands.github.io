...
function initWeb3() {
  if (typeof web3 !== 'undefined' && typeof web3.currentProvider !== 'undefined') {
    ...
    alert('请安装Metamask插件并连接至Ropsten网络。');
  }
  initVirtualLandsContractRopsten();
}


function initVirtualLandsContractRopsten () {
  // the file VirtualLands.json contains the ABI of the smart contract, and the address 
  // of the deployed instance of it 
  $.getJSON('VirtualLandsRopsten.json', function(data) {
    // Get the necessary contract artifact file and instantiate it with truffle-contract
    VTLContract = TruffleContract(data);

    // Set the provider for our contract
    VTLContract.setProvider(web3Provider);
...
function initCanvas () {
  ...
  $( "#removeAfterLoad" ).html('世外桃源  比邻而居（测试页面，正式版请移步至家园主页）');
}
...
function watchForWhenAnAccountIsUnlocked () {
  ...
  setTimeout(watchForWhenAnAccountIsUnlocked, 2000);  
}
